...
instance Assemble Pre.DesugaredExpr where
  ...
  assemble (Pre.DEApplication app [e1, e2@(Pre.DELiteral l)])
    | app == "+" || app == "add_int" = do
    case l of
      Cmm.LInt i -> do
        e1' <- assemble e1
        i' <- liftIO $ fetchConstant (Cmm.LInt i)
        pure (e1' <> [LLIR.Instruction (LLIR.AddConst i')])
      _ -> do
        e1' <- assemble e1
        e2' <- assemble e2
        pure (e1' <> e2' <> [LLIR.Instruction LLIR.Add])

  assemble (Pre.DEApplication app [e1@(Pre.DELiteral l), e2])
    | app == "+" || app == "add_int" = do
    case l of
      Cmm.LInt i -> do
        e2' <- assemble e2
        i' <- liftIO $ fetchConstant (Cmm.LInt i)
        pure (e2' <> [LLIR.Instruction (LLIR.AddConst i')])
      _ -> do
        e1' <- assemble e1
        e2' <- assemble e2
        pure (e1' <> e2' <> [LLIR.Instruction LLIR.Add])

  assemble (Pre.DEApplication app [e1, e2])
    | app == "add_int" = do
      e1' <- assemble e1
      e2' <- assemble e2
      pure (e1' <> e2' <> [LLIR.Instruction LLIR.Add])
  ...
  assemble (Pre.DEApplication app [e1, e2@(Pre.DELiteral l)])
    ...
    case l of
      Cmm.LInt i -> do
        e1' <- assemble e1
        i' <- liftIO $ fetchConstant (Cmm.LInt i)
        pure (e1' <> [LLIR.Instruction (LLIR.SubConst i')])
      _ -> do
        e1' <- assemble e1
        e2' <- assemble e2
        pure (e1' <> e2' <> [LLIR.Instruction LLIR.Sub])
